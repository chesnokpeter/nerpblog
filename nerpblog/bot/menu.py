...
class MenuManager:
    ...
    async def menu(self, offset: int, limit: int, **data) -> InlineKeyboardMarkup:
        ...
        if not k:
            ...
            k.append([InlineKeyboardButton(text='Меню 🏡', callback_data='menu')])
        elif not k and offset != 0:
            k.append([InlineKeyboardButton(text='Посты не найдены', callback_data='empty')])
            k.append([InlineKeyboardButton(text='◀️', callback_data='back_post'), InlineKeyboardButton(text='Меню 🏡', callback_data='menu')])
        elif len(list) < 5:
            k.append([InlineKeyboardButton(text='◀️', callback_data='back_post'), InlineKeyboardButton(text='Меню 🏡', callback_data='menu')])
        elif not offset <= 0:
            k.append([InlineKeyboardButton(text='◀️', callback_data='back_post'), InlineKeyboardButton(text='Меню 🏡', callback_data='menu'), InlineKeyboardButton(text='▶️', callback_data='next_post')])
        else:
            k.append([InlineKeyboardButton(text='Меню 🏡', callback_data='menu'), InlineKeyboardButton(text='▶️', callback_data='next_post')])
